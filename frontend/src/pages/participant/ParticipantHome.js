...
import EventSection from "../../components/participant/EventSection";
...
function ParticipantHome() {
  ...
  const allTournaments = useMemo(
    () => participantObj.tournamentArray || [],
    [participantObj.tournamentArray]
  );
  ...
  const filteredTournaments = useMemo(() => {
    ...
    return allTournaments.filter((t) => {
      const text = (t?.tournamentName || "").toLowerCase().trim();
      return text.includes(selectedCategory.toLowerCase().trim());
    });
      ...
      <section className="category-chip-section">
        ...
        <div className="category-chips">
          {categories.map((category) => (
            <button
              key={category}
              className={`category-chip ${selectedCategory === category ? "active-chip" : ""
                }`}
